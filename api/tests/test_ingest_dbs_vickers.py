...
def test_dbs_vickers_ingest_recovers_from_stale_csv_mapping(client: TestClient, db_engine):
    ...
    try:
        db.execute(
            text(
                "INSERT INTO accounts (id, name, platform, account_type, currency, country) VALUES "
                "(202, 'Test DBS Vickers Stale Map', 'DBS_VICKERS', 'BROKER', 'SGD', 'SG')"
            )
        )
        db.commit()
    finally:
    ...
    fixture = os.path.join(data_dir, "dbs_vickers_stale_map_fixture.xls")
    with open(fixture, "w", encoding="utf-8") as f:
        f.write(html)
    ...
    db = Session()
    try:
        # Simulate stale/incorrect historical registry mapping.
    ...
    with open(fixture, "rb") as f:
        resp = client.post(
            "/ingest/upload?account_id=202",
            files={"file": ("dbs_vickers_2026_04_09.xls", f, "application/vnd.ms-excel")},
        )
