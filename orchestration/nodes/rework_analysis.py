...
def run(state: GraphState) -> GraphState:
    ...
    effective_review = review_cycle.escalation_review or review_cycle.agent_review
    emit_stage_start(
        PipelineStage.REWORK_ANALYSIS,
        current_action="Analyzing review findings for the next rework cycle",
        evidence=[
            f"review_id={review_cycle.review_id}",
    ...
    emit_progress(
        ...
        evidence=[f"review_id={review_cycle.review_id}", f"model={cfg.builder_model}"],
